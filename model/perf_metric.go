...
func UpsertPerfMetric(metric *PerfMetric) error {
	...
	return DB.Clauses(clause.OnConflict{
		...
		DoUpdates: clause.Assignments(map[string]interface{}{
			...
			"total_tokens":     gorm.Expr("perf_metrics.total_tokens + ?", metric.TotalTokens),
...
func GetPerfMetricsSummaryAll(startTs int64, endTs int64, groups []string) ([]PerfMetricSummary, error) {
	...
	query := DB.Model(&PerfMetric{}).
		Select("model_name, SUM(request_count) as request_count, SUM(success_count) as success_count, SUM(total_latency_ms) as total_latency_ms, SUM(output_tokens) as output_tokens, SUM(input_tokens) as input_tokens, SUM(total_tokens) as total_tokens, SUM(cache_read_tokens) as cache_read_tokens, SUM(generation_ms) as generation_ms").
